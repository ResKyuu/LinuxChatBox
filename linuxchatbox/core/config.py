...
def save_config(opts, osc_port):
    """Persist DisplayOptions + OSC port to JSON."""
...
def load_config():
    """Load config from JSON. Returns (DisplayOptions, osc_port). Safe on first run."""
    from .display_options import DisplayOptions
    
    opts = DisplayOptions()
    osc_port = 9000
    if not CONFIG_PATH.exists():
        return opts, osc_port
    try:
